...
{
    [Authorize]
    public class WIMController : Controller
    {
        //
        // GET: /CCTVS/
        ...
        public ActionResult UpdateTable(int? page, int? pageListSize)
        {
            ...
            IEnumerable<PrimaryWStatus> pwss;
            ...
            {
                pwss = uOW.PrimaryWStatusRepository.Get(filter: s => s.PrimaryW.Name.ToUpper().Contains(searchString.ToUpper())
                                                                           || s.PrimaryWError.Name.ToUpper().Contains(searchString.ToUpper())
                                                                           || s.ContactName.ToUpper().Contains(searchString.ToUpper())
                                                                           || s.Details.ToUpper().Contains(searchString.ToUpper()),
                    includeProperties: "PrimaryW,PrimaryWError",
                    orderBy: s => s.OrderByDescending(x => x.DateOccur));
            }
            else
            {
                pwss = uOW.PrimaryWStatusRepository.Get(orderBy: s => s.OrderByDescending(x => x.DateOccur),
                includeProperties: "PrimaryW,PrimaryWError");
            }
            if (allEvent == null || allEvent == false)
            {
                pwss = pwss.Where(x => !x.Processed);
            }
            switch (sortOrder)
            {
                case "Error":
                    pwss = pwss.OrderBy(s => s.PrimaryWError.Name);
                    break;
                case "Error_desc":
                    pwss = pwss.OrderByDescending(s => s.PrimaryWError.Name);
                    break;

                case "Date_asc":
                    pwss = pwss.OrderBy(s => s.DateOccur);
                    break;
                default:
                    pwss = pwss.OrderByDescending(s => s.DateOccur);
                    break;
            }
            ...
            return PartialView("_UpdateTable", pwss.ToPagedList(pageNumber, pageSize));
        }
        public ActionResult Index(string sortOrder, string currentFilter, string searchString, int? page, int? pageListSize, bool? allEvent)
        {
            ...
            ViewBag.PageNumber = pageNumber;
            ViewBag.PageSize = pageSize;
            return View(pwss.ToPagedList(pageNumber, pageSize));
        }

        [Authorize(Roles = "Operator")]
        public ActionResult Create()
        {
            ViewBag.PrimaryWID = new SelectList(uOW.PrimaryWRepository.Get(filter: q => q.Disable == false), "PrimaryWID", "Name");
            ViewBag.PrimaryWErrorID = new SelectList(uOW.PrimaryWErrorRepository.dbSet, "PrimaryWErrorID", "Name");
        ...
        [HttpPost]
        [Authorize(Roles = "Operator")]
        public ActionResult Create([Bind(Include = "PrimaryWErrorID,Details,DateOccur,ContactName,OperatorName")] PrimaryWStatus pws,
            string[] PrimaryWID)
        {

            if (PrimaryWID == null)
            {
                ModelState.AddModelError("", "Chọn Trạm Báo lỗi");
                //return Json(new { success = false, message = "Create Unsuccessfully." });    
            }
            if (ModelState.IsValid)
            {
                foreach (var wID in PrimaryWID)
                {
                    var id = int.Parse(wID);
                    var pwssToADD = new PrimaryWStatus();
                    pwssToADD.PrimaryWID = id;
                    pwssToADD.PrimaryWErrorID = pws.PrimaryWErrorID;
                    pwssToADD.Details = pws.Details;
                    pwssToADD.DateOccur = pws.DateOccur;
                    pwssToADD.OperatorName = pws.OperatorName;
                    pwssToADD.ContactName = pws.ContactName;
                    pwssToADD.DateCreate = DateTime.Now;
                    pwssToADD.DateUpdate = DateTime.Now;
                    uOW.PrimaryWStatusRepository.Insert(pwssToADD);
                }
                
                uOW.Save();
                return Json(new { success = true, message = "Created Successfully." });    
            }
            ViewBag.PrimaryWID = new SelectList(uOW.CCTVRepository.Get(filter: q => q.Disable == false), "PrimaryWID", "Name", pws.PrimaryWID);
            ViewBag.PrimaryWErrorID = new SelectList(uOW.CCTVErrorRepository.dbSet, "PrimaryWErrorID", "Name", pws.PrimaryWErrorID);
            return PartialView("_Create");
        }
        public ActionResult GetDetails(int? id)
        {
            try
            {
                PrimaryWStatus pws = uOW.PrimaryWStatusRepository.Get(filter: x => x.PrimaryWStatusID == id,
                                includeProperties: "PrimaryW,PrimaryWError").FirstOrDefault();

                var rslt = new
                {
                    Name = pws.PrimaryW.Name,
                    Time = pws.DateOccur.ToString("dd-MM-yy HH:mm"),
                    Error = pws.PrimaryWError.Name
                };
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            try
            {
                ...
                PrimaryWStatus rslt = uOW.PrimaryWStatusRepository.Get(filter: x => x.PrimaryWStatusID == id,
                                includeProperties: "PrimaryW,PrimaryWError").Single();
                if (rslt != null)
                {
                    uOW.PrimaryWStatusRepository.Delete(rslt);
                    uOW.Save();
        ...
        public ActionResult Edit(int id = 0)
        {
            ...
            PrimaryWStatus pws = uOW.PrimaryWStatusRepository.GetByID(id);
            if (pws == null)
            {
                return HttpNotFound();
            }
            ViewBag.PrimaryWID = uOW.PrimaryWRepository.GetByID(pws.PrimaryWID).Name;
            ...
            ViewBag.PrimaryWErrorID = new SelectList(uOW.PrimaryWErrorRepository.dbSet, "PrimaryWErrorID", "Name", pws.PrimaryWErrorID);
            return PartialView("_Edit", pws);
        }
        ...
        [HttpPost]
        [Authorize(Roles = "Operator")]
        public ActionResult Edit(PrimaryWStatus pws, string submit)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    pws.Processed = true;
                    pws.DateUpdate = DateTime.Now;
                }
                uOW.PrimaryWStatusRepository.Update(pws);
                uOW.Save();
